				...
				if (pChip->streamCtl[stream - 1].pSubStream != NULL) {
					...
					if (pStream->runtime->status->state == SNDRV_PCM_STATE_RUNNING
						...
						if (pSel[indexVal] >= AUDIO_SINK_HANDSET && pSel[indexVal] < AUDIO_SINK_VALID_TOTAL) {
							...
							AUDIO_Ctrl_Trigger(ACTION_AUD_AddChannel, &ctl_parm.parm_spkr, NULL, 0);
						}
					}
				}
			}
			...
			if (indexVal != -1) {
				if (pChip->streamCtl[stream - 1].pSubStream != NULL) {
					...
					if (pStream->runtime->status->state == SNDRV_PCM_STATE_RUNNING
						...
						ctl_parm.parm_spkr.stream = (stream - 1);
						AUDIO_Ctrl_Trigger(ACTION_AUD_RemoveChannel, &ctl_parm.parm_spkr, NULL, 0);
					}
				}
...
static struct snd_kcontrol_new sgSndCtrls[] __devinitdata = {
	...
	BRCM_MIXER_CTRL_MISC(0, 0, "HW-CTL", AUDCTRL_HW_CFG_DSPMUTE,
			     CAPH_CTL_PRIVATE(CTL_STREAM_PANEL_MISC, 1,
				 CTL_FUNCTION_HW_CTL)),
	BRCM_MIXER_CTRL_MISC(0, 0, "HW-CTL", AUDCTRL_HW_CFG_DHAVEQ1,
			     CAPH_CTL_PRIVATE(CTL_STREAM_PANEL_MISC, 1,
				 CTL_FUNCTION_HW_CTL)),
	BRCM_MIXER_CTRL_MISC(0, 0, "HW-CTL", AUDCTRL_HW_CFG_DHAVEQ2,
			     CAPH_CTL_PRIVATE(CTL_STREAM_PANEL_MISC, 1,
				 CTL_FUNCTION_HW_CTL)),
