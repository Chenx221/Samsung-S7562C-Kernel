...
static void csl_caph_config_arm2sp(CSL_CAPH_PathID pathID)
{
	...
	if (p_arm2sp->mixMode == CSL_ARM2SP_VOICE_MIX_DL)
		...
		p_arm2sp->playbackMode = CSL_ARM2SP_PLAYBACK_DL;
	else if (p_arm2sp->mixMode == CSL_ARM2SP_VOICE_MIX_UL_NONE) {
		p_arm2sp->playbackMode = CSL_ARM2SP_PLAYBACK_UL;
		p_arm2sp->mixMode = CSL_ARM2SP_VOICE_MIX_NONE;
	}
		...
		pathID, int sinkNo, int blockPathIdx)
{
	...
	/* 44.1K ARM2SP, set the mixer gain */
		...
		CSL_CAPH_HWCTRL_CONFIG_t config)
{
	...
	if (path->sink[sinkNo] != CSL_CAPH_DEV_NONE) {
		...
		/*only mute it for mixing case to min impact*/
		if (csl_caph_FindPathWithSink(path->sink[sinkNo], pathID) != 0 && 
			path->source == CSL_CAPH_DEV_FM_RADIO) {
			int fm_wait = 235*1000; /*why FM must wait this long?*/
			csl_srcmixer_setMixInGain(path->srcmRoute[sinkNo][0].inChnl,
				path->srcmRoute[sinkNo][0].outChnl, GAIN_NA, GAIN_NA);
			aTrace(LOG_AUDIO_CSL, "%s fm_wait %d\n", __func__, fm_wait);
			usleep_range(fm_wait, fm_wait+20000);
		}
