...
#define sr352_DEFAULT_PIX_FMT		V4L2_PIX_FMT_UYVY	/* YUV422 */
#define sr352_DEFAULT_MBUS_PIX_FMT    V4L2_MBUS_FMT_UYVY8_2X8 
	...
	if (!state->pix.width || !state->pix.height) {
		...
		if (state->mode_switch == INIT_DONE_TO_CAMERA_PREVIEW) {
			...
			if(ANTI_BANDING_60HZ == camera_antibanding_get()){
				err = camdrv_ss_i2c_set_config_register(client, sr352_preview_60hz_setting, ARRAY_SIZE(sr352_preview_60hz_setting), "preview_60hz_setting");
				if (err < 0) {
					CAM_ERROR_PRINTK( "%s :sr352_preview_60hz_setting IS FAILED\n",__func__);
					return -EIO;
				}		
			}else{
				err = camdrv_ss_i2c_set_config_register(client, sr352_preview_50hz_setting, ARRAY_SIZE(sr352_preview_50hz_setting), "preview_50hz_setting");
				if (err < 0) {
					CAM_ERROR_PRINTK( "%s :sr352_preview_50hz_setting IS FAILED\n",__func__);
					return -EIO;
				}
			}
                                         ...
                                         regs_t reg_buffer[],int num_of_regs,char *name)
{
	...
	int index = 0;
	int err = 0;
	err = camdrv_ss_sr352_sensor_burst_write_list(client,reg_buffer,num_of_regs,name);
...
static int camdrv_ss_sr352_set_scene_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	CAM_INFO_PRINTK( "%s : value =%d\n", __func__, ctrl->value);
		...
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_portrait_regs, ARRAY_SIZE(sr352_scene_portrait_regs), "scene_portrait_regs");
			break;
		}
	
		case SCENE_MODE_NIGHTSHOT:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_nightshot_regs, ARRAY_SIZE(sr352_scene_nightshot_regs), "scene_nightshot_regs");
			break;
		}
	
		case SCENE_MODE_BACK_LIGHT:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_backlight_regs, ARRAY_SIZE(sr352_scene_backlight_regs), "scene_backlight_regs");
			break;
		}
	
		case SCENE_MODE_LANDSCAPE:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_landscape_regs, ARRAY_SIZE(sr352_scene_landscape_regs), "scene_landscape_regs");
			break;
		}
	
		case SCENE_MODE_SPORTS:
		{
				err = camdrv_ss_i2c_set_config_register(client,sr352_scene_sports_regs, ARRAY_SIZE(sr352_scene_sports_regs), "scene_sports_regs");
			break;
		}
	
		case SCENE_MODE_PARTY_INDOOR:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_party_indoor_regs, ARRAY_SIZE(sr352_scene_party_indoor_regs), "scene_party_indoor_regs");
			break;
		}
	
		case SCENE_MODE_BEACH_SNOW:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_beach_snow_regs, ARRAY_SIZE(sr352_scene_beach_snow_regs), "scene_beach_snow_regs");
			break;
		}
	
		case SCENE_MODE_SUNSET:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_sunset_regs, ARRAY_SIZE(sr352_scene_sunset_regs), "scene_sunset_regs");
			break;
		}
	
		case SCENE_MODE_DUSK_DAWN:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_duskdawn_regs,  ARRAY_SIZE(sr352_scene_duskdawn_regs), "scene_duskdawn_regs");
			break;
		}
	
		case SCENE_MODE_FALL_COLOR:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_fall_color_regs, ARRAY_SIZE(sr352_scene_fall_color_regs), "scene_fall_color_regs");
			break;
		}
	
		case SCENE_MODE_FIREWORKS:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_fireworks_regs,  ARRAY_SIZE(sr352_scene_fireworks_regs), "scene_fireworks_regs");
			break;
		}
	
		case SCENE_MODE_TEXT:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_text_regs,  ARRAY_SIZE(sr352_scene_text_regs), "scene_text_regs");
			break;
		}
	
		case SCENE_MODE_CANDLE_LIGHT:
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_candle_light_regs, ARRAY_SIZE(sr352_scene_candle_light_regs), "scene_candle_light_regs");
			break;
		}
...
//	sensor->get_light_condition   = camdrv_ss_sr352_get_light_condition;
	...
	sensor->set_scene_mode 			   = camdrv_ss_sr352_set_scene_mode;   //denis
	...
	sensor->preview_50hz_setting 			  = sr352_preview_50hz_setting;
	sensor->rows_num_preview_50hz_setting 	  = ARRAY_SIZE(sr352_preview_50hz_setting);
	sensor->preview_60hz_setting 			  = sr352_preview_60hz_setting;
	sensor->rows_num_preview_60hz_setting 	  = ARRAY_SIZE(sr352_preview_60hz_setting);
